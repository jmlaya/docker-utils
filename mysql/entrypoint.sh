...
if [ -z "${ADMIN_USER_DEFAULT}" ]; then
    ADMIN_USER_DEFAULT="admin"
fi

echo "Default admin user: $ADMIN_USER_DEFAULT"

if [ -z "${ADMIN_USER_DEFAULT_PASSWORD}" ]; then
    ADMIN_USER_DEFAULT_PASSWORD="admin"
fi

echo "Default password for admin user: $ADMIN_USER_DEFAULT_PASSWORD"

if [ -z "${DB_DEFAULT_PASSWORD}" ]; then
    DB_DEFAULT_PASSWORD="admin"
fi

echo "Default db password: $DB_DEFAULT_PASSWORD"
...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    echo "mysql_install_db --datadir=${MYSQL_DATA_DIR} --user=${MYSQL_USER} >/dev/null 2>&1"
    ...
    echo "Creating ${ADMIN_USER_DEFAULT} user..."
    mysql -uroot -e "GRANT ALL PRIVILEGES on *.* TO '${ADMIN_USER_DEFAULT}'@'%' IDENTIFIED BY '${ADMIN_USER_DEFAULT_PASSWORD}' WITH GRANT OPTION;"
...
create_users_and_databases() {
  ...
  if [ -n "${DB_NAMES}" ]; then
    ...
    mysql --defaults-file=/etc/mysql/debian.cnf \
    -e "GRANT ALL PRIVILEGES ON \`$db\`.* TO '${DB_USER}' IDENTIFIED BY '${DB_DEFAULT_PASSWORD}';"
